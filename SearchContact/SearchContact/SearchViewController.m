...
@property(nonatomic,strong) NSMutableArray *searchList;///搜索结果数组
...
- (void)viewDidLoad {
    ...
    self.ptableView.tableHeaderView=self.searchController.searchBar;
...
- (void)updateSearchResultsForSearchController:(UISearchController *)searchController
{
    ...
    self.searchList=[SearchResultHandle getResultByArray:self.dataArray keywords:searchStr];
    self.searchList=[self.searchList filterElement];
    NSMutableArray *tempArray=[NSMutableArray array];
    [self.searchList enumerateObjectsUsingBlock:^(NSString* obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ContactModel *model=[[ContactModel alloc] init];
        model.nickName=obj;
        [tempArray addObject:model];
    }];
    self.searchResultController.dataSource=tempArray;
...
-(UISearchController*)searchController{
    if (!_searchController) {
        ...
        [_searchController.searchBar setBackgroundImage:[self imageWithColor:[UIColor clearColor] size:_searchController.searchBar.bounds.size]];
